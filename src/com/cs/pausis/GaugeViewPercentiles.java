...
public class GaugeViewPercentiles extends View {
	...
	public static final float SCALE_END_VALUE = 100f;
	...
	public GaugeViewPercentiles(final Context context, final AttributeSet attrs, final int defStyle) {
		super(context, attrs, defStyle);
		readAttrs(context, attrs, defStyle);
		init();
	}

	public GaugeViewPercentiles(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public GaugeViewPercentiles(final Context context) {
		this(context, null, 0);
	}

	private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
		final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GaugeViewStandardDeviations, defStyle, 0);
		mShowOuterShadow = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showOuterShadow, SHOW_OUTER_SHADOW);
		mShowOuterBorder = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showOuterBorder, SHOW_OUTER_BORDER);
		mShowOuterRim = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showOuterRim, SHOW_OUTER_RIM);
		mShowInnerRim = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showInnerRim, SHOW_INNER_RIM);
		mShowNeedle = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showNeedle, SHOW_NEEDLE);
		mShowScale = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showScale, SHOW_SCALE);
		mShowRanges = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showRanges, SHOW_RANGES);
		mShowText = a.getBoolean(R.styleable.GaugeViewStandardDeviations_showText, SHOW_TEXT);

		mOuterShadowWidth = mShowOuterShadow ? a.getFloat(R.styleable.GaugeViewStandardDeviations_outerShadowWidth, OUTER_SHADOW_WIDTH) : 0.0f;
		mOuterBorderWidth = mShowOuterBorder ? a.getFloat(R.styleable.GaugeViewStandardDeviations_outerBorderWidth, OUTER_BORDER_WIDTH) : 0.0f;
		mOuterRimWidth = mShowOuterRim ? a.getFloat(R.styleable.GaugeViewStandardDeviations_outerRimWidth, OUTER_RIM_WIDTH) : 0.0f;
		mInnerRimWidth = mShowInnerRim ? a.getFloat(R.styleable.GaugeViewStandardDeviations_innerRimWidth, INNER_RIM_WIDTH) : 0.0f;
		mInnerRimBorderWidth = mShowInnerRim ? a.getFloat(R.styleable.GaugeViewStandardDeviations_innerRimBorderWidth, INNER_RIM_BORDER_WIDTH) : 0.0f;

		mNeedleWidth = a.getFloat(R.styleable.GaugeViewStandardDeviations_needleWidth, NEEDLE_WIDTH);
		mNeedleHeight = a.getFloat(R.styleable.GaugeViewStandardDeviations_needleHeight, NEEDLE_HEIGHT);

		mScalePosition = (mShowScale || mShowRanges) ? a.getFloat(R.styleable.GaugeViewStandardDeviations_scalePosition, SCALE_POSITION) : 0.0f;
		mScaleStartValue = a.getFloat(R.styleable.GaugeViewStandardDeviations_scaleStartValue, SCALE_START_VALUE);
		mScaleEndValue = a.getFloat(R.styleable.GaugeViewStandardDeviations_scaleEndValue, SCALE_END_VALUE);
		mScaleStartAngle = a.getFloat(R.styleable.GaugeViewStandardDeviations_scaleStartAngle, SCALE_START_ANGLE);

		mDivisions = a.getInteger(R.styleable.GaugeViewStandardDeviations_divisions, SCALE_DIVISIONS);
		mSubdivisions = a.getInteger(R.styleable.GaugeViewStandardDeviations_subdivisions, SCALE_SUBDIVISIONS);

		if (mShowRanges) {
			mTextShadowColor = a.getColor(R.styleable.GaugeViewStandardDeviations_textShadowColor, TEXT_SHADOW_COLOR);
			final int rangesId = a.getResourceId(R.styleable.GaugeViewStandardDeviations_rangeValues, 0);
			final int colorsId = a.getResourceId(R.styleable.GaugeViewStandardDeviations_rangeColors, 0);
			readRanges(context.getResources(), rangesId, colorsId);
		}

		if (mShowText) {
			final int textValueId = a.getResourceId(R.styleable.GaugeViewStandardDeviations_textValue, 0);
			final String textValue = a.getString(R.styleable.GaugeViewStandardDeviations_textValue);
			mTextValue = (0 < textValueId) ? context.getString(textValueId) : (null != textValue) ? textValue : "";

			final int textUnitId = a.getResourceId(R.styleable.GaugeViewStandardDeviations_textUnit, 0);
			final String textUnit = a.getString(R.styleable.GaugeViewStandardDeviations_textUnit);
			mTextUnit = (0 < textUnitId) ? context.getString(textUnitId) : (null != textUnit) ? textUnit : "";
			mTextValueColor = a.getColor(R.styleable.GaugeViewStandardDeviations_textValueColor, TEXT_VALUE_COLOR);
			mTextUnitColor = a.getColor(R.styleable.GaugeViewStandardDeviations_textUnitColor, TEXT_UNIT_COLOR);
			mTextShadowColor = a.getColor(R.styleable.GaugeViewStandardDeviations_textShadowColor, TEXT_SHADOW_COLOR);

			mTextValueSize = a.getFloat(R.styleable.GaugeViewStandardDeviations_textValueSize, TEXT_VALUE_SIZE);
			mTextUnitSize = a.getFloat(R.styleable.GaugeViewStandardDeviations_textUnitSize, TEXT_UNIT_SIZE);
		}
	...
	private void readRanges(final Resources res, final int rangesId, final int colorsId) {
		if (rangesId > 0 && colorsId > 0) {
			...
			mRangeValues = new int[length];
			mRangeColors = new int[length];
			for (int i = 0; i < length; i++) {
				mRangeValues[i] = Integer.parseInt(ranges[i]);
	...
	public void setDefaultRanges() {
		mRangeValues = new int[] { 5, 25, 50, 75, 100 };
		mRangeColors = new int[]{ Color.rgb(231, 32, 43), Color.rgb(232, 111, 33), Color.rgb(232, 186, 33), Color.rgb(232, 231, 33), Color.rgb(21, 176, 26) };
	}
	...
	private void drawScale(final Canvas canvas) {
		...
		for (int i = 0; i < totalTicks; i++) {
			...
			//Since the scale is meant to be 7 values only i.e from 5th to 95th percentile, 
			//therefore the 100 was divided into 5 places which resulted in the ticked values below
			...
			if (value == 0) {//5th
				...
				canvas.drawText("5th", 0.5f, y3 + 0.045f, paint);
			}
			else if (value == 20.0) {//25th
				// Draw a division tick
				canvas.drawLine(0.5f, y1, 0.5f, y3, paint);
				// Draw the text 0.15 away from the division tick
				canvas.drawText("25th", 0.5f, y3 + 0.045f, paint);
			}
			else if (value == 40.0) {//50th
				// Draw a division tick
				canvas.drawLine(0.5f, y1, 0.5f, y3, paint);
				// Draw the text 0.15 away from the division tick
				canvas.drawText("50th", 0.5f, y3 + 0.045f, paint);
			}
			else if (value == 60.0) {//75th
				// Draw a division tick
				canvas.drawLine(0.5f, y1, 0.5f, y3, paint);
				// Draw the text 0.15 away from the division tick
				canvas.drawText("75th", 0.5f, y3 + 0.045f, paint);
			}
			else if (value == 80.0) {//95th
				// Draw a division tick
				canvas.drawLine(0.5f, y1, 0.5f, y3, paint);
				// Draw the text 0.15 away from the division tick
				canvas.drawText("95th", 0.5f, y3 + 0.045f, paint);
			}
			else if (value > 80.0f) {//if value is greater than 85.8 stop ticking the canvas
				//break;
			}
			else {
